...
function _subscribeEvents() {
  getPublisherSubscriber().subscribe('view/stage/abstract/render/template/perform', function (topic, args) {
    ...
    if (args[0].get('type') === 'TypolinkCheckbox') {
      // formElement.set('label', 'xxx');
      // if (typeof formElement.get('properties') === 'object') {
      //   let overrideLabel = formElement.get('properties').linkLabel || 'x';
      //   console.log('overrideLabel', formElement);
      //   formElement.set('label', overrideLabel);
      // }
      // let overrideLabel = typeof formElement.get('properties') === 'object' ? formElement.get('properties').linkLabel : 'x';
      // formElement.set('label', overrideLabel);
...
function renderTypolinkEditor(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName) {
  // console.log(editorConfiguration, editorHtml, collectionElementIdentifier, collectionName);
